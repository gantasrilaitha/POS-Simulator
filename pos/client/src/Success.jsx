...
const Success = () => {
  ...
  const handleAddButtonClick = (productId) => {
    ...
    axios
    .post(`http://localhost:3001/updateQuantity/${productId}`, { quantity })
    .then((response) => {
      // After successfully updating the quantity, fetch the updated product data
      axios
        .get(`http://localhost:3001/images/${productId}`)
        .then((response) => {
  ...
  const handleClearButtonClick = () => {
    ...
    axios
      .post('http://localhost:3001/saveBillingData', billingData)
      .then((response) => {
